...
const PassportLoginPlugin = graphile_utils_1.makeExtendSchemaPlugin(build => ({
  ...
  `,
    resolvers: {
        Mutation: {
            ...
            from new_user, new_session`, [username, email, name, avatarUrl, password]);
                    ...
                    if (details.session_id) {
                        ...
                        await pgClient.query(`select set_config('jwt.claims.session_id', $1, true)`, [details.session_id]);
                    ...
                    const safeErrorCodes = [
                        ...
                        "LOCKD",
                        "EMTKN",
                        ...Object.keys(handleErrors_1.ERROR_MESSAGE_OVERRIDES),
                    ];
                    if (safeErrorCodes.includes(code)) {
                        throw e;
                    }
                    else {
                        console.error("Unrecognised error in PassportLoginPlugin; replacing with sanitised version");
                        console.error(e);
            ...
            async login(_mutation, args, context, resolveInfo) {
                ...
                try {
                    // Call our login function to find out if the username/password combination exists
                    const { rows: [session], } = await rootPgPool.query(`select sessions.* from app_private.login($1, $2) sessions where not (sessions is null)`, [username, password]);
                    if (!session) {
                    ...
                    await pgClient.query(`select set_config('jwt.claims.session_id', $1, true)`, [session.uuid]);
                    // Fetch the data that was requested from GraphQL, and return it
                    const sql = build.pgSql;
                    const [row] = await selectGraphQLResultFromTable(sql.fragment `app_public.users`, (tableAlias, sqlBuilder) => {
...
exports.default = PassportLoginPlugin;
//# sourceMappingURL=PassportLoginPlugin.js.map