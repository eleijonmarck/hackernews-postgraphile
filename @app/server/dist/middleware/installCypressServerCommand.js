...
exports.default = (app) => {
     ...
     * function.
     */
    const handleCypressServerCommand = async (req, res, next) => {
        /*
    ...
    app.get("/cypressServerCommand", body_parser_1.urlencoded({ extended: false }), handleCypressServerCommand);
};
async function runCommand(req, res, rootPgPool, command, payload) {
    if (command === "clearTestUsers") {
        ...
        const { username = "testuser", email = `${username}@example.com`, verified = false, name = username, avatarUrl = null, password = "TestUserPassword", } = payload;
        ...
        const userEmailId = userEmailSecrets.user_email_id;
        if (!verified) {
        ...
        const { username = "testuser", email = `${username}@example.com`, verified = false, name = username, avatarUrl = null, password = "TestUserPassword", next = "/", } = payload;
        const user = await reallyCreateUser(rootPgPool, {
            username,
            email,
            verified,
...
async function reallyCreateUser(rootPgPool, { username, email, verified, name, avatarUrl, password, }) {
    const { rows: [user], } = await rootPgPool.query(`SELECT * FROM app_private.really_create_user(
        username := $1,
        email := $2,
        email_is_verified := $3,
        name := $4,
...
async function createSession(rootPgPool, userId) {
    const { rows: [session], } = await rootPgPool.query(`
...
async function getUserEmailSecrets(rootPgPool, email) {
    const { rows: [userEmailSecrets], } = await rootPgPool.query(`
      select *
...
//# sourceMappingURL=installCypressServerCommand.js.map