...
const plugins_1 = require("postgraphile/plugins");
const graphile_build_1 = require("graphile-build");
const pg_pubsub_1 = tslib_1.__importDefault(require("@graphile/pg-pubsub"));
const pg_simplify_inflector_1 = tslib_1.__importDefault(require("@graphile-contrib/pg-simplify-inflector"));
const pro_1 = tslib_1.__importDefault(require("@graphile/pro")); // Requires license key
const PassportLoginPlugin_1 = tslib_1.__importDefault(require("../plugins/PassportLoginPlugin"));
const PrimaryKeyMutationsOnlyPlugin_1 = tslib_1.__importDefault(require("../plugins/PrimaryKeyMutationsOnlyPlugin"));
const SubscriptionsPlugin_1 = tslib_1.__importDefault(require("../plugins/SubscriptionsPlugin"));
const handleErrors_1 = tslib_1.__importDefault(require("../utils/handleErrors"));
const app_1 = require("../app");
const installDatabasePools_1 = require("./installDatabasePools");
const path_1 = require("path");
const TagsFilePlugin = plugins_1.makePgSmartTagsFromFilePlugin(
// We're using JSONC for VSCode compatibility; also using an explicit file
// path keeps the tests happy.
path_1.resolve(__dirname, "../../postgraphile.tags.jsonc"));
...
//const isTest = process.env.NODE_ENV === "test";
const pluginHook = postgraphile_1.makePluginHook([
    // Add the pub/sub realtime provider
    pg_pubsub_1.default,
    // If we have a Graphile Pro license, then enable the plugin
    ...(process.env.GRAPHILE_LICENSE ? [pro_1.default] : []),
]);
function getPostGraphileOptions({ websocketMiddlewares, rootPgPool, }) {
    const options = {
        ...
        handleErrors: handleErrors_1.default,
        /*
        ...
        appendPlugins: [
            ...
            PrimaryKeyMutationsOnlyPlugin_1.default,
            // Adds the `login` mutation to enable users to log in
            PassportLoginPlugin_1.default,
            // Adds realtime features to our GraphQL schema
            SubscriptionsPlugin_1.default,
        ],
        /*
        ...
        async pgSettings(req) {
            ...
            if (sessionId) {
                ...
                await rootPgPool.query("UPDATE app_private.sessions SET last_active = NOW() WHERE uuid = $1 AND last_active < NOW() - INTERVAL '15 seconds'", [sessionId]);
            }
            return {
        ...
        defaultPaginationCap: parseInt(process.env.GRAPHQL_PAGINATION_CAP || "", 10) || 50,
        graphqlDepthLimit: parseInt(process.env.GRAPHQL_DEPTH_LIMIT || "", 10) || 12,
        graphqlCostLimit: parseInt(process.env.GRAPHQL_COST_LIMIT || "", 10) || 30000,
        exposeGraphQLCost: (parseInt(process.env.HIDE_QUERY_COST || "", 10) || 0) < 1,
    };
    return options;
}
exports.getPostGraphileOptions = getPostGraphileOptions;
function installPostGraphile(app) {
    const websocketMiddlewares = app_1.getWebsocketMiddlewares(app);
    const authPgPool = installDatabasePools_1.getAuthPgPool(app);
    const rootPgPool = installDatabasePools_1.getRootPgPool(app);
    const middleware = postgraphile_1.postgraphile(authPgPool, "app_public", getPostGraphileOptions({
        websocketMiddlewares,
        rootPgPool,
    }));
    app.set("postgraphileMiddleware", middleware);
    app.use(middleware);
    const httpServer = app_1.getHttpServer(app);
    if (httpServer) {
        postgraphile_1.enhanceHttpServerWithSubscriptions(httpServer, middleware);
    }
}
exports.default = installPostGraphile;
//# sourceMappingURL=installPostGraphile.js.map